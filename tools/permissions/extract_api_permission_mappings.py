...
MAPPINGS_MODULE_PATH = "../../androguard/core/api_specific_resources/api_permission_mappings/"
...
api_specific_mappings_module_name = "%s_api%s.py" % (
    MAPPINGS_MODULE_NAME, API_VERSION)
...
if os.path.exists(api_specific_mappings_module_path):
    ...
    print("If you want create a file for newer version, please, delete file: %s" %
          api_specific_mappings_module_path)
    exit(1)


print("Reading method mapping file...")
pscout_method_mapping_filepath = os.path.join(
    PATH_TO_PSCOUT_FOLDER, PSCOUT_METHOD_MAPPING_FILENAME)
...
for line in methods_mapping_file_lines:
    ...
    if line.startswith("Permission:"):
        ...
        method_identificator = "%s-%s-(%s)%s" % (transformClassParam(
            className), methodName, modParStr, transformClassParam(returnValue))
...
for line in contentproviderfields_mapping_file_lines:
    ...
    if line.startswith("PERMISSION:"):
        ...
        field_identificator = "%s-%s-%s" % (transformClassParam(
            className), name, transformClassParam(proto))
