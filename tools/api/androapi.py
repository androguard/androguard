...
def extractInformation(filename):
    ...
    package = filename[filename.find(
        "reference/android/"):][10:-5].replace("//", "/")
            ...
            for package in PERMS_API[i]:
                ...
                for j in PERMS_API[i][package]:
                    if isinstance(j, Function):
                        fd.write(
                            "\t\t[\"F\"," "\"" + j.name + "\"," + "\"" + j.desc_return + "\"]" + ",\n")
                    else:
                        fd.write(
                            "\t\t[\"C\"," "\"" + j.name + "\"," + "\"" + j.desc_return + "\"]" + ",\n")
...
# extractInformation( BASE_DOCS +
# "docs/reference/android/net/sip/SipAudioCall.html" )
# #android/accounts/Account.html" )
# #"docs/reference/android/accounts/AccountManager.html" )
...
# i.findNextSlibing(text=re.compile("READ_PHONE_STATE"))

# for i in soup.findAll(text=re.compile("READ_PHONE_STATE")):
# print i, i.parent.name, i.findPrevious(re.compile('^A')),
# i.findPreviousSibling(re.compile('^A'))
