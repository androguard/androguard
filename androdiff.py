...
option_1 = {'name': ('-t', '--threshold'),
            'help': 'define the threshold', 'nargs': 1}
option_2 = {'name': ('-c', '--compressor'),
            'help': 'define the compressor', 'nargs': 1}
option_3 = {'name': ('-d', '--display'),
...
option_5 = {'name': ('-e', '--exclude'),
            'help': 'exclude specific class name (python regexp)', 'nargs': 1}
option_6 = {'name': (
    '-s', '--size'), 'help': 'exclude specific method below the specific size', 'nargs': 1}
option_7 = {'name': ('-v', '--version'),
...
def main(options, arguments):
    ...
    if options.input != None:
        ...
        el = elsim.Elsim(ProxyDalvik(d1, dx1), ProxyDalvik(
            d2, dx2), FS, threshold, options.compressor)
        el.show()

        e1 = elsim.split_elements(el, el.get_similar_elements())
        for i in e1:
            j = e1[i]
            elb = elsim.Elsim(ProxyDalvikMethod(i), ProxyDalvikMethod(
                j), FILTERS_DALVIK_BB, threshold, options.compressor)
            # elb.show()
