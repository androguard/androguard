...
def get_field_ast(field: dex.EncodedField) -> dict[str, Any]:
...
class DvMethod:
    ...
    start_block: DEXBasicBlock | None
    cls_name: str
    name: str
    lparams: list[int] | None
    var_to_name: dict[int, Variable]
    writer: Writer | None
    graph: Graph | None
    ...
    def get_source_ext(self) -> list[tuple[str, ...]]:
        if self.writer:
...
class DvClass:
    ...
    def get_ast(self) -> dict[str, Any]:
        ...
        for m in self.methods:
            if isinstance(m, DvMethod) and m.ast is not None:
    ...
    def get_source_ext(self) -> list[tuple[str, Any]]:
        ...
        list_proto: list[tuple[str, Any] | tuple[str, Any, Any]] = [
