...
class Writer(object):
    ...
    def write_method(self):
        ...
        for modifier in access:
            if modifier == 'constructor':
                ...
                continue
            acc.append(modifier)
        self.write_ext(('NEWLINE', '\n%s' % (self.space()))) # see later write below
        if self.constructor:
    ...
    def visit_decl(self, var):
        if not var.declared:
            ...
            self.write('%s%s v%s' % (
                       self.space(), get_type(var_type), var.value()), data="DECLARATION")
            self.end_ins()
    ...
    def visit_variable(self, var):
        ...
        self.write_ext(('NAME_VARIABLE', 'v%s' % var.value(), var, var_type))
    ...
    def visit_check_cast(self, arg, atype):
        self.write('(checkcast)(', data="CHECKCAST")
        arg.visit(self)
        self.write(', %s)' % atype, data="CHECKCAST_END")
    ...
    def visit_fill_array(self, array, value):
        ...
        data = value.get_data()
        self.write(', '.join(['%d' % ord(c) for c in data[:-1]]), data="COMMA")
