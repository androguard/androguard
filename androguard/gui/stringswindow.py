...
class StringsValueWindow(QtGui.QTreeView):
    ...
    def __init__(self, parent=None, win=None, session=None):
        ...
        self.model = QtGui.QStandardItemModel(
            self.session.get_nb_strings(), 4, self)
        ...
        for digest, filename, strings_analysis in self.session.get_strings():
            for string_value in strings_analysis:
                ...
                self.model.setData(self.model.index(row, 1, QtCore.QModelIndex()), len(
                    strings_analysis[string_value].get_xref_from()))
                self.model.setData(self.model.index(
                    row, 2, QtCore.QModelIndex()), filename)
                self.model.setData(self.model.index(
                    row, 3, QtCore.QModelIndex()), digest)
                self.reverse_strings[
                    repr(string_value) + digest] = strings_analysis[string_value]
                row += 1
    ...
    def slotDoubleClicked(self, mi):
        ...
        if column == 0:
            xwin = XrefDialogString(parent=self.mainwin, win=self.mainwin, string_analysis=self.reverse_strings[
                                    self.model.item(row).text() + self.model.item(row, 3).text()])
            xwin.show()
