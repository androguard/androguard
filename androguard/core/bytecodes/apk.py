            ...
            while nb < len(self.packages[package_name]):
                ...
                if isinstance(header, ARSCHeader):
                    if header.type == RES_TABLE_TYPE_TYPE:
                        ...
                        nb_i = 0
                        xml_name = "%s/%s.xml" % (("values-" + a_res_type.config.get_config_name()) if a_res_type.config.get_config_name() else "values",a_res_type.get_type())
                        self.values_list[xml_name] = []
                        for entry, res_id in entries:
                            if entry != -1:
                                ate = self.packages[package_name][nb + 3 + nb_i]
                                self.values_list[xml_name].append({"id":ate.mResId,"name":ate.get_value(),"value": ate.get_key_data() if a_res_type.get_type() in {"string","drawable","mipmap"} else None})
                                nb_i += 1
                        nb += 3 + nb_i - 1
                nb += 1
