...
option_9 = { 'name' : ('-o', '--output'), 'help' :'print output to a file', 'nargs' : 1 }


options = [option_0, option_1, option_2, option_3, option_4, option_5, option_6, option_8, option_9]
...
def BuildCFG(inputFile, outputFile ) :
    ...
    for clsItem in d.get_classes():
        fd.write("[currClass]=" + clsItem.get_name() + " : [parentClass]=" + clsItem.get_superclassname() + "\n")
        for method in clsItem.get_methods():
...
def main(options, arguments):
    if options.shell != None:
        interact()
    elif options.input !=None and options.output != None :
         d = BuildCFG(options.input, options.output)
