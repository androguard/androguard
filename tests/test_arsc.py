...
option_0 = {'name': ('-d', '--directory'),
            'help': 'directory input', 'nargs': 1}
option_1 = {'name': ('-v', '--verbose'),
            'help': 'add debug', 'action': 'count'}
...
def test(got, expected):
    ...
    print('%s got: %s expected: %s' %
          (prefix, repr(got), repr(expected)), end=' ')
    return (got == expected)
...
class AndroLog(object):
    ...
    def dump(self, msg):
        ...
        logger.info(str_date + "%s[%d]: %s" %
                    (self.filename, self.id_file, msg))

    def error(self, msg):
        now = datetime.datetime.now()
        str_date = now.strftime("%Y-%m-%d %H:%M:%S ")
        logger.info(str_date + "ERROR %s[%d]: %s" %
                    (self.filename, self.id_file, msg))
        import traceback
        traceback.print_exc()
